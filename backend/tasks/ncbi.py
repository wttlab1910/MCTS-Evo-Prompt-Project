...
try:
    ...
    try:
        ...
        def load_dataset(name, name2=None):
            """简单的备用load_dataset实现"""
            print(f"使用最小备用load_dataset: {name}")
            ...
            if name == "ncbi_disease":
                return {
                    ...
                    "validation": [
                        {"tokens": ["Cystic", "fibrosis", "gene", "mutation"],
                         "ner_tags": [1, 2, 0, 0]}
                    ]
                }
            # 需要时为其他数据集添加更多情况
...
class CustomDataLoader:
    ...
    def _collate_fn(self, batch_data):
        ...
        question, answers = zip(*[(item['question'], item['answer']) for item in batch_data])
...
class CustomTask(BaseTask):
    ...
    def load_task_dataset(self, data_dir):
        ...
        # Convert dataset to standard format for processing
        if isinstance(dataset, dict):
            ...
            for split_name, split_data in dataset.items():
                ...
                # Filter examples with non-empty ner_tags
                filtered_split = []
                for example in split_data:
                    if example and "ner_tags" in example and len(example.get("ner_tags", [])) > 0:
                        filtered_split.append(example)
                
                if filtered_split:
                    transformed_dataset[split_name] = filtered_split
            
            # Make sure we have the required splits
            if "train" not in transformed_dataset:
                transformed_dataset["train"] = []
            if "test" not in transformed_dataset and "validation" in transformed_dataset:
                transformed_dataset["test"] = transformed_dataset["validation"]
            elif "test" not in transformed_dataset:
                transformed_dataset["test"] = []
            
            return transformed_dataset
        
        # Return the original dataset if it's not in the expected format
        return dataset
    ...
    def transform_format(self, data_dict):
        ...
        for split_name, data_split in data_dict.items():
            ...
            # Handle dictionary data structure (our custom format)
            ...
            # Handle HuggingFace dataset format
            elif hasattr(data_split, 'column_names'):
                ...
                print(f"Unrecognized data format for {split_name}")
